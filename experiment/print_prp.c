...
void print_prp(Building* B_mainb, Possible_Relative_Position prp) {
	...
	int w_area = max_coordinate_x - min_coordinate_x;
	int l_area = max_coordinate_y - min_coordinate_y;

	char ***area = malloc(sizeof(char**)* w_area);

	int i,j;

	for(i=0; i<w_area; i++) {
		area[i] = malloc(sizeof(char*) * l_area);
	}
	for(i=0; i<w_area; i++) {
		for(j=0; j<l_area; j++) {
			area[i][j] = malloc(100);
	...
	for(i = x_mainb; i < x_mainb + w_mainb; i++) {
		for(j = y_mainb; j < y_mainb + l_mainb; j++) {
	...
	for(i = x_e1; i < x_e1 + w_e1; i++) {
		for(j = y_e1; j < y_e1 + l_e1; j++) {
	...
	for(i = x_e2; i < x_e2 + w_e2; i++) {
		for(j = y_e2; j < y_e2 + l_e2; j++) {
