...
Specialtown **dictionary_specialtown = malloc(sizeof(*SpecialTown) * NUMSPECIALTOWN);
...
void put_data_of_specialtown_to_dictionary() {
	...
	for(i=0; i<NUMSPECIALTOWN; i++) {
		...
		for(j=0; j<3; j++) {
			dictionary_specialtown.trio_building_compose_specialtown[j] = search_building_from_dictionary(building_name_buffer[j]);
		}
	}
