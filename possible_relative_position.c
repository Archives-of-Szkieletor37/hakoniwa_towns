...
Possible_Relative_Position *make_possible_relative_position(SpecialTown *specialtown, Point *point_e1, Point *point_e2, int cnt_dominated_mainbs_adjacent_square) {
	...
	if (prp != NULL) {
		...
		if(DS_specialtown == NULL || possible_point_x1 == NULL || possible_point_x2 == NULL) {
			free(prp);	
			return NULL;
		}
	}
...
extern LinkedList_Possible_Relative_Position possible_relative_position_init(SpecialTown* p_DS_specialtown, Point possible_point) {
...
/*
 * int put_2nd_building_adjacently_on_top(Building *p_B_object, Building *p_B_target, SpecialTown *p_DS_specialtown, LinkedList_Possible_Relative_Position *p_LinkedList_possible_relative_position) {
...
void put_adjacently_pattern_a(Building *p_B_target, Building *p_B_object, Building *p_B_element_2, Point *p_LinkedList_object_possible_point) {
	...
	for(i=(p_B_target->left_bottom_point.x + 1 - p_B_object->B_width); i < (p_B_target->left_bottom_point.x + p_B_target->B_width); i++) {
		Point p;
		p = point_init(i, p_B_target->left_bottom_point.y + p_B_target->B_length);

		if (check_overlap(p, p_B_object->B_length, p_B_object->B_width, p_B_element_2) == true && is_adjacent(p_B_object, p_B_element_2) == false) {
			*(p_LinkedList_object_possible_point + cnt_object_possible_point).x = i;
			*(p_LinkedList_object_possible_point + cnt_object_possible_point).y = p_B_target->left_bottom_point.y + p_B_target->B_length;
	...
	for(i=(p_B_target->left_bottom_point.x + 1 - p_B_object->B_width); i < (p_B_target->left_bottom_point.x + p_B_target->B_width); i++) {
		Point p;
		p = point_init(i, p_B_target->left_bottom_point.y - p_B_object->B_length);
	...
	for(i=(p_B_target->left_bottom_point.y + 1 - p_B_object->B_length); i < (p_B_target->left_bottom_point.y + p_B_target->B_width); i++) {
		Point p;
		p = point_init(p_B_target->left_bottom_point.x - p_B_object->B_width, i);
	...
	for(i=(p_B_target->left_bottom_point.y + 1 - p_B_object->B_length); i < (p_B_target->left_bottom_point.x + p_B_target->B_width); i++) {
		Point p;
		p = point_init(p_B_target->left_bottom_point.x + p_B_target->B_width, i);
...
//	int cnt_LinkedList_possible_relative_position = 0;
	...
	for(l=0; l<2; l++) {
		...
		*/
		...
		int cnt_e1_possible_point = put_adjacently_pattern_a(p_B_mainb, p_B_element_1, building_null(), p_LinkedList_e1_possible_point);
